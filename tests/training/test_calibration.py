...
from fklearn.training.calibration import isotonic_calibration_learner, find_thresholds_with_same_risk


def test_isotonic_calibration_learner():
    df_train = pd.DataFrame({"id": ["id1", "id2", "id3", "id4"], "pred": [0.25, 0.64, 0.12, 0.9], "y": [0, 1, 0, 1]})

    df_test = pd.DataFrame(
        {"id": ["id4", "id4", "id5", "id6", "id7"], "pred": [0.55, 0.12, 0.13, 0.9, 0.95], "y": [1, 0, 0, 1, 1]}
    )
...
def test_find_thresholds_with_same_risk():
    df_with_ecdf = pd.DataFrame(
        ...
        columns=["sensitive_factor", "target", "prediction_ecdf", "unfair_band"],
    )

    df_expected = df_with_ecdf.copy()
    df_expected["fair"] = pd.Series([2, 3, 4, 1, 4, 4, 3, None, 2, 1])
    fair_thresholds = {"group_2": [-1, 105.0, 152.0, 328.0, 670.0], "group_1": [-1, 305.0, 416.0, 635.0, 672.0]}

    learner = find_thresholds_with_same_risk(
        sensitive_factor="sensitive_factor", unfair_band_column="unfair_band", model_prediction_output="prediction_ecdf"
    )
