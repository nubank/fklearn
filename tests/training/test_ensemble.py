...
def test_xgb_octopus_classification_learner():
    ...
    extra_params_by_bin = {
        split_bin: {"reg_alpha": 0.0, "colsample_bytree": 0.3, "subsample": 0.7, "reg_lambda": 3}
        for split_bin in train_split_bins
    }

    features_by_bin = {1: ["x1"], 2: ["x1", "x2"]}

    train_fn = xgb_octopus_classification_learner(
        learning_rate_by_bin=learning_rate_by_bin,
