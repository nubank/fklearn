...
def test_elasticnet_regression_learner():
    ...
    learner = elasticnet_regression_learner(
        features=["x1", "x2"], target="y", params=None, prediction_column="prediction", weight_column="w"
    )
...
def test_custom_supervised_model_learner():
    ...
    df_train_classification = pd.DataFrame(
        {"id": ["id1", "id2", "id3", "id4"], "x1": [10.0, 13.0, 10.0, 13.0], "x2": [0, 1, 1, 0], "y": [0, 1, 0, 1]}
    )
    df_test_classification = pd.DataFrame(
        {"id": ["id4", "id4", "id5", "id6"], "x1": [12.0, 1000.0, -4.0, 0.0], "x2": [1, 1, 0, 1], "y": [1, 0, 0, 1]}
    )
