...
def test_area_under_the_cumulative_gain_curve():
    ...
    result = area_under_the_cumulative_gain_curve(
        df, prediction="x", outcome="y", treatment="t", min_rows=3, steps=df.shape[0], effect_fn=linear_effect
    )
    assert round(result, 3) == 1.529
...
def test_area_under_the_relative_cumulative_gain_curve():
    ...
    result = area_under_the_relative_cumulative_gain_curve(
        df, prediction="x", outcome="y", treatment="t", min_rows=3, steps=df.shape[0], effect_fn=linear_effect
    )
    assert round(result, 3) == 0.344
