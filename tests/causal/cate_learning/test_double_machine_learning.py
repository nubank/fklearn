...
def test__cv_estimate():
    ...
    np.random.seed(123)
    y_pred, models = _cv_estimate(model=LinearRegression(), train_data=df_train, features=["x"], y="y", n_splits=3)
...
def test_non_parametric_double_ml_learner():
    ...
    np.random.seed(123)
    predict_fn, pred_train, log = non_parametric_double_ml_learner(
        df=df_train,
        feature_columns=["x"],
        treatment_column="t",
        outcome_column="y",
    ...
    expected_test_pred = pd.DataFrame(dict(x=[1, 2, 3], test_prediction=[1.0, 2.0, 3.0]))
