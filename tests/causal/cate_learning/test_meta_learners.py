...
from fklearn.causal.cate_learning.meta_learners import TREATMENT_FEATURE, _append_treatment_feature, \
    _get_learner_features, \
    _get_unique_treatments, _filter_by_treatment, _create_treatment_flag, _fit_by_treatment,\
...
def test__get_unique_treatments():
    ...
    assert _get_unique_treatments(df, treatment_col="treatment", control_name="control") == ["treatment_A",
                                                                                             "treatment_B",
                                                                                             "treatment_C"]
...
def test__filter_by_treatment():
    ...
    expected_df: DataFrame = pd.DataFrame(data=expected_values, columns=["feat1", "treatment", "target"])

    assert _filter_by_treatment(df,
                                treatment_col="treatment",
                                treatment_name=selected_treatment,
                                control_name="control") == expected_df
...
def test__create_treatment_flag():
    ...
    assert _create_treatment_flag(df, treatment_col="group", treatment_name="treatment") == expected_df
...
def test__fit_by_treatment():
    ...
    learners, logs = _fit_by_treatment(df,
...
def test__predict_by_treatment_flag_positive():
    ...
    prediction_array = _predict_by_treatment_flag(df,
                                                  learner_fcn=predict_fn,
                                                  is_treatment=True,
