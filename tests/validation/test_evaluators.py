...
def test_combined_evaluators():
    predictions = pd.DataFrame({"target": [0, 1, 2], "prediction": [0.5, 0.9, 1.5]})
...
def test_mean_prediction_evaluator():
    predictions = pd.DataFrame({"prediction": [1, 0.9, 40]})

    eval_fn = mean_prediction_evaluator(prediction_column="prediction", eval_name="eval_name")
...
def test_auc_evaluator():
    predictions = pd.DataFrame({"target": [0, 1, 0, 1], "prediction": [0.2, 0.9, 0.3, 0.3]})

    eval_fn = auc_evaluator(prediction_column="prediction", target_column="target", eval_name="eval_name")
...
def test_auc_evaluator_with_weights():
    predictions = pd.DataFrame(
        {
            "target": [0, 1, 0, 1],
            "prediction": [0.2, 0.9, 0.3, 0.3],
            "weights": [1, 1, 1, 3],
        }
    )

    eval_fn = auc_evaluator(
        prediction_column="prediction", target_column="target", weight_column="weights", eval_name="eval_name"
    )
...
def test_roc_auc_evaluator():
    ...
    eval_fn = roc_auc_evaluator(prediction_column="prediction", target_column="target", eval_name="eval_name")
...
def test_pr_auc_evaluator():
    ...
    eval_fn = pr_auc_evaluator(prediction_column="prediction", target_column="target", eval_name="eval_name")
...
def test_precision_evaluator():
    ...
    eval_fn = precision_evaluator(
        prediction_column="prediction", threshold=0.5, target_column="target", eval_name="eval_name"
    )
...
def test_fbeta_score_evaluator():
    predictions = pd.DataFrame({"target": [0, 1, 0, 1], "prediction": [0.2, 0.9, 0.3, 0.8]})

    eval_fn = fbeta_score_evaluator(
        prediction_column="prediction", threshold=0.5, beta=1, target_column="target", eval_name="eval_name"
    )
...
def test_logloss_evaluator():
    ...
    eval_fn = logloss_evaluator(prediction_column="prediction", target_column="target", eval_name="eval_name")
...
def test_brier_score_evaluator():
    ...
    eval_fn = brier_score_evaluator(prediction_column="prediction", target_column="target", eval_name="eval_name")
...
def test_binary_calibration_evaluator():
    ...
    predictions = pd.DataFrame({"target": target, "prediction": probs})

    eval_fn = expected_calibration_error_evaluator(
        prediction_column="prediction", target_column="target", eval_name="eval_name", n_bins=100, bin_choice="count"
    )
    ...
    eval_fn = expected_calibration_error_evaluator(
        prediction_column="prediction", target_column="target", eval_name="eval_name", n_bins=100, bin_choice="prob"
    )
...
def test_correlation_evaluator():
    predictions = pd.DataFrame({"target": [0, 1, 2], "prediction": [0.5, 1.0, 1.5]})
...
def test_linear_coefficient_evaluator():
    predictions = pd.DataFrame({"target": [1, 2, 3], "prediction": [2, 4, 6]})
...
def test_ndcg_evaluator(exponential_gain):
    predictions = pd.DataFrame({"target": [1.0, 0.5, 1.5], "prediction": [0.9, 0.3, 1.2]})

    k_raises = [-1, 0, 4]
    for k in k_raises:
        with pytest.raises(ValueError):
            ndcg_evaluator(predictions, k=k, exponential_gain=exponential_gain)

    k_not_raises = [None, 1, 2, 3]
    for k in k_not_raises:
        result = ndcg_evaluator(predictions, k=k, exponential_gain=exponential_gain)
        assert result["ndcg_evaluator__target"] == 1.0


def test_split_evaluator():
    predictions = pd.DataFrame(
        {"split_col_a": [1, 1, 0], "split_col_b": [2, 0, 0], "target": [0, 1, 2], "prediction": [0.5, 0.9, 1.5]}
    )
...
def test_permutation_evaluator():
    ...
    pimp1 = permutation_evaluator(
        test_df, fake_predict, base_eval, features=["a"], baseline=True, shuffle_all_at_once=False
    )

    assert pimp1["permutation_importance"]["a"] == expected_results
    assert pimp1["permutation_importance_baseline"] == expected_results

    pimp2 = permutation_evaluator(
        test_df, fake_predict, base_eval, features=["a", "bb"], baseline=False, shuffle_all_at_once=False
    )

    assert pimp2["permutation_importance"]["a"] == expected_results
    assert pimp2["permutation_importance"]["bb"] == expected_results

    pimp3 = permutation_evaluator(
        test_df, fake_predict, base_eval, features=["a", "bb"], baseline=True, shuffle_all_at_once=True
    )
    ...
    test_df2 = pd.DataFrame(
        {"abc": np.linspace(0, 1, 100), "abcd": 1.0 - np.linspace(0, 1, 100), "target": np.ones(100)}
    )
    ...
    pimp4 = permutation_evaluator(
        test_df2, fake_predict2, base_eval, features=["abc"], baseline=True, shuffle_all_at_once=False, random_state=0
    )
...
def test_hash_evaluator():
    ...
    df2 = df1.copy().sample(frac=1.0).reset_index(drop=True)
    ...
    eval_fn = hash_evaluator(hash_columns=["feature1", "featureCategorical"], eval_name="eval_name")
    # evaluate hash in all columns
    eval_fn_all = hash_evaluator(eval_name="eval_name")
    eval_fn_order = hash_evaluator(
        hash_columns=["feature1", "featureCategorical"], eval_name="eval_name", consider_index=True
    )
...
def test_exponential_coefficient_evaluator():
    ...
    predictions = pd.DataFrame(dict(prediction=prediction, target=np.exp(a0 + a1 * prediction)))
...
def test_logistic_coefficient_evaluator():
    ...
    predictions = pd.DataFrame(dict(prediction=[1, 1, 1, 2, 2, 2, 3, 3, 3], target=[0, 0, 0, 0, 0, 0, 1, 1, 1]))
