...
def test_random_search_tuner(tmpdir):
    train_set = pd.DataFrame(
        {
            ...
            "date": pd.to_datetime(
                ["2016-02-01", "2016-03-01", "2016-05-01", "2016-06-01", "2016-07-01", "2016-08-01"]
            ),
            "x": [0.9, 0.3, 0.3, 0.4, 0.4, 0.5],
            "target": [1, 0, 1, 0, 1, 1],
        }
    )
    ...
    split_fn = out_of_time_and_space_splitter(
        n_splits=2, in_time_limit="2016-05-01", space_column="id", time_column="date"
    )

    tuning_log = random_search_tuner(
        space=space,
        train_set=train_set,
...
def test_grid_search_tuner(tmpdir):
    ...
    space = {"learning_rate": lambda: [1e-3, 1e-2, 1e-1], "num_estimators": lambda: [1, 2], "silent": lambda: [True]}
    ...
    tuning_log = grid_search_cv(
        space=space, train_set=train_set, param_train_fn=param_train_fn, split_fn=split_fn, eval_fn=eval_fn
    )

    assert len(tuning_log) == 3 * 2

    space = {"learning_rate": lambda: [1e-3, 1e-2, 1e-1, 1], "num_estimators": lambda: [1, 2], "silent": lambda: [True]}
