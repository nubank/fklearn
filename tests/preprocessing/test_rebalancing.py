...
def test_rebalance_by_continuous():
    ...
    result1 = (
        rebalance_by_continuous(data2, "col2", buckets=3, by_quantile=False).sort_values("col2").reset_index(drop=True)
    )
    ...
    result1 = (
        rebalance_by_continuous(data2, "col2", buckets=3, by_quantile=True).sort_values("col2").reset_index(drop=True)
    )
