...
def test_time_split_dataset(test_df=df):
    in_time_train_set, out_time_test_set = time_split_dataset(
        dataset=test_df,
    ...
    expected_train = pd.DataFrame(
        {"space": ["space1", "space2"], "time": [pd.to_datetime("2016-10-01"), pd.to_datetime("2016-10-01")]}
    )

    expected_test = pd.DataFrame(
        {"space": ["space1", "space2"], "time": [pd.to_datetime("2016-11-01"), pd.to_datetime("2016-11-01")]}
    )
    ...
    expected_test = pd.DataFrame(
        {"space": ["space1", "space2"], "time": [pd.to_datetime("2016-12-01"), pd.to_datetime("2016-12-01")]}
    )
...
def test_space_time_split_dataset(
    test_df=df, test_df_with_new_id=df_with_new_id, test_df_only_one_point_per_id=df_only_one_point_per_id
):

    train_set, intime_outspace_hdout, outtime_inspace_hdout, outtime_outspace_hdout = space_time_split_dataset(
        dataset=test_df,
        train_start_date="2016-10-01",
        train_end_date="2016-11-01",
        holdout_end_date="2016-12-01",
    ...
    expected_train = pd.DataFrame({"space": ["space2"], "time": [pd.to_datetime("2016-10-01")]})

    expected_intime_outspace_holdout = pd.DataFrame({"space": ["space1"], "time": [pd.to_datetime("2016-10-01")]})

    expected_outtime_outspace_holdout = pd.DataFrame({"space": ["space1"], "time": [pd.to_datetime("2016-11-01")]})

    expected_outtime_inspace_holdout = pd.DataFrame({"space": ["space2"], "time": [pd.to_datetime("2016-11-01")]})
    ...
    expected_outtime_outspace_holdout = pd.DataFrame({"space": ["space1"], "time": [pd.to_datetime("2016-12-01")]})

    expected_outtime_inspace_holdout = pd.DataFrame({"space": ["space2"], "time": [pd.to_datetime("2016-12-01")]})
    ...
    expected_outtime_outspace_holdout = pd.DataFrame(
        {"space": ["space1", "space3"], "time": [pd.to_datetime("2016-11-01"), pd.to_datetime("2016-11-01")]}
    )
    ...
    train_set, intime_outspace_hdout, outtime_inspace_hdout, outtime_outspace_hdout = space_time_split_dataset(
        dataset=test_df_only_one_point_per_id,
        train_start_date="2016-10-01",
        train_end_date="2016-11-01",
        holdout_end_date="2016-12-01",
        split_seed=1,
    ...
    expected_outtime_outspace_holdout = pd.DataFrame(
        {"space": ["space3", "space4"], "time": [pd.to_datetime("2016-11-01"), pd.to_datetime("2016-11-01")]}
    )
...
@given(
    sample=gen_stratified_test_data(),
...
def test_stratified_split_dataset(sample, random_state, test_size):
    ...
    train_data, test_data = stratified_split_dataset(
        expected_data, target_column_name, test_size=test_size, random_state=random_state
    )
